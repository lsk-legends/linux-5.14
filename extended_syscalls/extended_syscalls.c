		...
		prefetch_swapin_num, int __user *, hit_on_prefetch_num)
{
	...
	dmd_swapin_num = get_adc_profile_counter(ADC_ONDEMAND_SWAPIN);
	prf_swapin_num = get_adc_profile_counter(ADC_PREFETCH_SWAPIN);
	hit_prftch_num = get_adc_profile_counter(ADC_HIT_ON_PREFETCH);
	swapout_num = get_adc_profile_counter(ADC_SWAPOUT);

	report_adc_profile_counters();
